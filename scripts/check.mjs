 ...
 *   node scripts/check.mjs --changed           Affected-only mode: tsc --build is already
 *                                              incremental, vitest gets --changed origin/main
 *
 * Exit codes:
 *   0  all checks passed
...
/**
 * Apply runtime flags to a check's args. Keeps the CHECKS catalogue declarative
 * while allowing flags like --changed to inject tool-specific options.
 */
function apply_runtime_flags(check, values) {
  if (values.changed && check.name === 'test') {
    return { ...check, args: [...check.args, '--changed', 'origin/main'] };
  }
  return check;
}
...
async function main() {
  ...
  for (const raw of selected) {
    const check = apply_runtime_flags(raw, values);
    if (!values.json) {
